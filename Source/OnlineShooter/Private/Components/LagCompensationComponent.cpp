	...
	float HitTime)
{
	...
	{
		const float DamageToCause = Confirm.bHeadShot ? Character->GetEquippedWeapon()->GetCritDamage() : Character->GetEquippedWeapon()->GetDamage();
	...
	const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime, AWeapon* DamageCauser)
{
	...
	for (auto& HitCharacter : HitCharacters)
	{
		...
		{
			float HeadshotDamage = Confirm.HeadShots[HitCharacter] * HitCharacter->GetEquippedWeapon()->GetCritDamage();
	...
	)
{
	...
	{
		// Debug
		if (PathResult.HitResult.Component.IsValid() && bDebug)
		{
...
void ULagCompensationComponent::ProjectileServerScoreRequest_Implementation(AOnlineShooterCharacter* HitCharacter,
